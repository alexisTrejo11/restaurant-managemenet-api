...
class PaymentQuerySet(models.QuerySet):
    ...
    def dynamic_search(self, search_params: dict):
        ...
        if search_params.get("amount_min"):
            try:
                queryset = queryset.filter(
                    total__gte=Decimal(search_params["amount_min"])
                )
            except:
                pass
        ...
        if search_params.get("amount_max"):
            try:
                queryset = queryset.filter(
                    total__lte=Decimal(search_params["amount_max"])
                )
            except:
                pass
...
class Payment(models.Model):
    ...
    order = models.OneToOneField(
        "orders.Order", on_delete=models.PROTECT, null=True, related_name="payment"
    )
